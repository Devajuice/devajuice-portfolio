...
export function useTimezone() {
  const MY_TZ = 'Asia/Kolkata', MY_TZ_SHORT = 'IST';
  const [display, setDisplay] = useState({ time: '--:-- --', label: 'Loading...' });
  ...
  useEffect(() => {
    ...
    const getMinutes = (tz) => {
      try {
        ...
        return parseInt(p.find(x => x.type === 'hour')?.value||0)*60 + parseInt(p.find(x => x.type === 'minute')?.value||0);
      } catch { return 0; }
    };
    const getLabel = () => {
      try {
        const isDev = visitorTz === MY_TZ || visitorTz.includes('Calcutta') || visitorTz.includes('Kolkata');
...
export function useParticleCanvas(canvasRef) {
  useEffect(() => {
    ...
    const MAX_DIST = 140, MOUSE_DIST = 180;
    ...
    const createParticles = () => {
      particles = Array.from({ length: COUNT }, () => ({
        ...
        r: Math.random()*1.8+0.6, opacity: Math.random()*0.5+0.2, pulse: Math.random()*Math.PI*2,
      }));
    };
    const getAccentColor = () => document.documentElement.getAttribute('data-theme') === 'dark' ? '59,130,246' : '37,99,235';
    const draw = () => {
      ...
      for (const p of particles) {
        ...
        const po = p.opacity*(0.7+0.3*Math.sin(p.pulse)), pr = p.r*(0.9+0.1*Math.sin(p.pulse*1.3));
        ctx.beginPath(); ctx.arc(p.x, p.y, pr, 0, Math.PI*2);
        ctx.fillStyle = `rgba(${accent},${po})`; ctx.fill();
      }
      for (let i = 0; i < particles.length; i++) {
        for (let j = i+1; j < particles.length; j++) {
          ...
          if (dist < MAX_DIST) {
            ...
            ctx.strokeStyle = `rgba(${accent},${(1-dist/MAX_DIST)*0.18})`; ctx.lineWidth = 0.8; ctx.stroke();
          }
        }
      }
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (!del) {
        ...
        if (cIdx === word.length) { del = true; timer = setTimeout(tick, 1800); return; }
        timer = setTimeout(tick, 90);
      } else {
        setText(word.slice(0, --cIdx));
        if (cIdx === 0) { del = false; wIdx = (wIdx+1)%words.length; timer = setTimeout(tick, 300); return; }
