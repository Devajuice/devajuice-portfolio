...
const BEATS_PER_CHORD = 8,
  TOTAL_BEATS = SONG.length * BEATS_PER_CHORD;
const MELODY_CYCLES = [
  {
    2: 523.25,
    4: 587.33,
...
function scheduleHihat(startTime, vol) {
  const ctx = _getCtx();
  if (!bgDryGain) return;
  // Allocate the noise buffer once; reuse on every tick (same pattern as _clickBuf)
  if (!_hihatBuf || _hihatBuf.sampleRate !== ctx.sampleRate) {
    const bufLen = Math.floor(ctx.sampleRate * 0.05);
    _hihatBuf = ctx.createBuffer(1, bufLen, ctx.sampleRate);
