export default async function handler(req, res) {
  ...
  const svg = `<svg width="1200" height="630" viewBox="0 0 1200 630" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="98" y="71" text-anchor="middle" font-size="13" font-weight="700" fill="#3b82f6">&lt;/&gt;</text>

    <!-- Domain -->
    <text x="128" y="72" font-size="18" fill="#ffffff" fill-opacity="0.45">devajuice.vercel.app</text>
    ...
    <text x="118" y="483" font-size="15" fill="#ffffff" fill-opacity="0.7">Available for work</text>

    <!-- Title -->
    <text x="80" y="560" font-size="96" font-weight="700" letter-spacing="-2" fill="url(#titleGrad)">${escapeXml(title)}</text>

    <!-- Subtitle -->
    <text x="80" y="600" font-size="26" fill="#ffffff" fill-opacity="0.5">${escapeXml(subtitle)}</text>

    ${renderTags(["Python", "JavaScript", "React", "Data Analysis"])}
  </svg>`;

  const resvg = new Resvg(svg, { loadSystemFonts: true });
...
function renderTags(tags) {
  ...
  for (let i = tags.length - 1; i >= 0; i--) {
    ...
    <text x="${x + py}" y="547" font-size="${fs}" font-weight="700"
